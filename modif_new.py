...
def modif_titlo_on_start(w, p):
    w = repl(w, modif_lib_new.titlo_on_start, 'match')

    # Случаи с неоднозначным раскрытием
    w = repl_var(w, p, modif_lib_new.titlo_on_start_var, 'match')
...
def modif_not_on_end(w):
    for coll in modif_lib_new.not_on_end:
        mo = re.search(coll, w)

        if mo and mo.end() != real_len(w):
            w = re.sub(coll, modif_lib_new.not_on_end[coll], w, 1)
...
def modif_varia(w):
    return repl(w, modif_lib_new.varia)
...
def modif_on_end(w, p):
    for coll in modif_lib_new.on_end:
        mo = re.search(coll + '$', w)

        if mo and mo.end() == real_len(w):
            w = w[:mo.start()] + re.sub(coll, modif_lib_new.on_end[coll], w[mo.start():])

    for coll in modif_lib_new.on_end_var:
        mo = re.search(coll + '$', w)

        if mo and mo.end() == real_len(w):
            if p in modif_lib_new.on_end_var[coll]:
                w = w[:mo.start()] + re.sub(coll, modif_lib_new.on_end_var[coll][p], w[mo.start():])
            else:
                w = w[:mo.start()] + re.sub(coll, modif_lib_new.on_end_var[coll][''], w[mo.start():])
...
# Исправление ошибок
def modif_errors(w):
    w = repl(w, modif_lib_new.errors)
    w = repl(w, modif_lib_new.errors_on_start, 'match')
